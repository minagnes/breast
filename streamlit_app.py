...
# -*- coding: utf-8 -*-
"""
유방영상의학과 근무 스케줄 생성기 - Streamlit 버전 (streamlit_app.py)
================================================================
generate_schedule.py 의 배정 로직을 그대로 옮기고, 터미널 input()/print() 를
Streamlit 화면(버튼·업로드·다운로드)으로 바꾼 버전.

실행
----
    pip install streamlit openpyxl holidays
    streamlit run streamlit_app.py

화면 흐름 (터미널판의 4단계와 동일)
------------------------------------
    1) 연도/월 입력 → [휴가 입력표 생성] 버튼
       → 공휴일은 holidays 라이브러리로 자동 조회(질문 없음)
       → 화면에 뜨는 [휴가 입력표 다운로드] 버튼으로 vacation_YYYY-MM.xlsx 를 받는다.
    2) 받은 엑셀을 열어 K1~K5(K3 제외)/F1~F4/J1·J2/R 각자의 휴가·학회·출장을
       해당 날짜 칸에 입력하고 저장한다.
    3) 저장한 파일을 화면의 업로드 창에 그대로 올린다.
    4) [스케줄 생성] 버튼을 누르면 업로드한 휴가 내용을 반영한
       스케줄(본 스케줄 + VIP 시트)을 만들고, 다운로드 버튼과 검증 결과를 보여준다.

기준 문서: 스케줄_배정_규정.docx
K4·K5 고정 배정 규칙:
    - K5: 오전 Mammo 화·금 고정 / 오후 Breast US 화·금 고정 / 오후 Mammo2 월·수·목 고정
    - K4: 오후 Mammo2 월·화·금 고정 / 오후 Breast US 수 고정
    - K2: 오후 Mammo2 화·수·금 고정(대체 후보에서 제외)
"""
...
# ------------------------------------------------------------
# 아래는 화면에서 연/월/공휴일/휴가가 확정되면 채워진다 (모듈 로드시엔 빈 값)
# ------------------------------------------------------------
YEAR = None
MONTH = None
HOLIDAYS = {}
...
def get_holidays_for_month(year, month):
    """holidays 라이브러리로 대한민국 공휴일(대체공휴일 포함)을 자동 조회."""
    try:
        import holidays as holidays_lib
    except ImportError:
        st.error(
            "공휴일 자동 조회에 필요한 'holidays' 패키지가 설치되어 있지 않습니다.\n"
            "터미널에서 `pip install holidays` 실행 후 앱을 다시 시작하세요."
        )
        st.stop()
...
def parse_vacation_file(file_obj, year, month):
    """업로드된 vacation 엑셀(파일 객체)을 읽어 (이름, date, 구분, 라벨) 리스트로 변환."""
    ...
    if ws.max_column < 1 + len(dates):
        st.warning(f"업로드한 파일의 열 개수가 예상({len(dates)}개)보다 적습니다. "
                   f"연/월이 휴가표를 만들 때와 같은지 확인하세요.")
    ...
    if missing_names:
        st.warning(f"업로드한 파일에서 다음 근무자를 찾지 못했습니다: {', '.join(missing_names)}")
...
def assign_pm(d):
    ...
    if wd in (1, 2, 3) and not duties["breast"]:
        # K2는 화/수/금 오후 Mammo2 고정이라 대체 후보에서 제외 - K1처럼 대체 없이 유지
...
def save_workbook(year, month, day_data, weeks):
    """본 스케줄 + VIP 시트가 담긴 Workbook 을 만들어 돌려준다 (파일로 저장하지 않음)."""
    wb = Workbook()
    ws = wb.active
...
def render_app():
    ...
    if generate_clicked:
        if "year" not in st.session_state:
            ...
            vacations = parse_vacation_file(uploaded, year_i, month_i)
            VACATIONS = vacations
            VAC_INDEX = {}
            for (nm, dt_, period, label) in vacations:
